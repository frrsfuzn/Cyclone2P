...
unsigned long previousMillisQuit=0;
...
void mainMenu(){
  ...
  Serial.println(animasiMainMenu);
  if(counter==8){
    naik=false;
  }else if(counter==0){
    naik=true;
...
void play(){
  ...
  */
  ...
  if (resetMillisPlay){
    previousMillisQuit=currentMillis;
  }
  if(val1==0&&val2==0){
    resetMillisPlay=false;
    aman=false;
    runningPlay=false;
    // Untuk ke MainMenu
    if(currentMillis-previousMillisQuit>=intervalToQuit){
      previousMillisQuit=currentMillis;
      gamePlay=false;
      animasiMainMenu=true;
